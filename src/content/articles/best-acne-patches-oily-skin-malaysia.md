...
Prices below are **approximate, so always check the current listing**, and on Shopee, TikTok Shop and Lazada compare **per-patch, not per-pack**, since pack sizes differ wildly.
...
- **STIK Original Dot (~RM7-10 for 15, approximate).** A hydrocolloid that comes in multiple sizes (useful for matching patch to spot and minimising edge) and carries salicylic acid, niacinamide and tea tree. On a per-patch basis it sits at the lower-cost end alongside Nexcare, which matters when oily skin means changing patches more often. Sold mainly on the marketplaces; buy from the official store.
